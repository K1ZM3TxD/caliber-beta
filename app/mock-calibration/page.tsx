...
export default function MockCalibrationPage() {
  ...
  async function postEvent(sid: string, event: any) {
  ...
  async function fetchResult(sid: string) {
    appendLog("Fetching result...");
    const res = await fetch(`/api/calibration/result?calibrationId=${sid}`);
  ...
  async function pollForResult(sid: string, maxTries = 10, delayMs = 1500) {
    appendLog("Polling for result...");
    for (let i = 0; i < maxTries; i++) {
      try {
        const res = await fetchResult(sid);
        if (res && res.result) {
      ...
      <div className="w-full max-w-[720px] px-6">
        <div className="relative min-h-[560px]">
          <div className="h-full w-full flex flex-col items-center justify-center text-center">
            <div className="font-semibold tracking-tight text-4xl mb-2" style={{ color: "#F2F2F2" }}>
              Calibration Runner
            </div>
