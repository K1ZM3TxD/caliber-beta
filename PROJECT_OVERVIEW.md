...
Calibration prepares the signal. The extension is where users evaluate real jobs.
...
- **Caliber** is the decision layer (where users understand fit).

The strongest insight happens when the calibration pattern is applied against a real job. Calibration alone is not the full experience.

---

## Calibration Results Page Role

The calibration results page should be **focused and minimal**:
- Calibration completion confirmation
- Title anchors (the 3 recommended titles)
- Transition into extension usage (CTA to LinkedIn)

The results page should **not** over-explain with weak summary sections that restate prompt content. Sections like "Where You Operate Best" and "Lose Energy" have been intentionally removed — the real insight appears in the extension when applied to live jobs.
