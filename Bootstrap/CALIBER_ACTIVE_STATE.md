...
- **SGD scoring-keyword injection + result display** (2026-03-15): Signal labels are now mapped via SIGNAL_SCORING_KEYWORDS to actual title-scoring vocabulary terms. Keywords are repeated 2x to pass extractBroadTokens count≥2 gate and receive PROMPT_SOURCE_WEIGHT=2.5 in anchor extraction. Validation logging (JSON) confirms baseline vs new title. Result page shows "Signals influencing this calibration" when user selected Yes. Files: `lib/calibration_machine.ts`, `app/calibration/page.tsx`.
