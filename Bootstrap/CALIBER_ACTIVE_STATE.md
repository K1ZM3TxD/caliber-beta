...
- **Better Search Trigger (BST)** — two-phase surface-classification-driven recovery suggestion (v0.9.17) with session-level title dedup preventing loops. Self-suggestion suppression (v0.9.7): BST will not render if suggested title equals current query. Weak-job sidecard click respects surface classification state. **Classification model (v0.9.17):** Phase 1 (<5 scored) = no banner. Phase 2 (5–9 scored, provisional) = healthy requires ≥2 strong or ≥1 at 8.0+; BST on 0 strong; neutral on exactly 1 job in [7.0, 7.9]. Phase 3 (10+ scored) = forced from evidence. BST and healthy-surface banners mutually exclusive. Validation instrumentation active (DOM hydration observer + surface-validation logger). **Previous blocker** (issue #72, premature refresh-time evaluation) fixed via `initialSurfaceResolved` gate. **Current validation** (issue #81): classification thresholds re-locked, DOM hydration confirmed incremental. **Return-loop fix** (issue #82): sidecard BST path now checks `bstTitleAlreadySeen()` and calls `bstMarkSuggested()` before showing; BST link click immediately records in `bstSearchedQueries`; `showPrescanBSTBanner()` has final loop-guard safety net; full `[decision-trace]` logging on all selection/fallback paths. BST is NOT yet fully passed — validation pending with instrumentation data. **Empty banner fix** (issue #83): `showPrescanBSTBanner()` now hard-rejects null/empty titles at entry, preventing empty "TRY THIS TITLE INSTEAD" banners from rendering.
...
- **BST empty suggestion banner fix** (2026-03-18): FIX. `showPrescanBSTBanner()` now rejects null/empty/whitespace-only titles at entry — no DOM manipulation occurs. Exhausted-candidate caller no longer invokes with null. Generic recovery branch (banner shell without link) removed. Pre-render debug log added. Issue #83. Files: `extension/content_linkedin.js`.
- **BST return-loop fix** (2026-03-18): FIX. Three gaps in BST loop prevention closed: (1) sidecard BST path now checks `bstTitleAlreadySeen()` + calls `bstMarkSuggested()` (previously bypassed both). (2) BST link click immediately records title in `bstSearchedQueries` (previously delayed to next Phase 2). (3) `showPrescanBSTBanner()` has final safety-net loop guard. Full `[decision-trace]` logging added to `determinePrescanSuggestion` and all fallback paths. Issue #82. Files: `extension/content_linkedin.js`.
- **BST surface classification re-lock + validation instrumentation** (2026-03-18, v0.9.17): BREAK+UPDATE. Binary banner logic replaced with two-phase classification: Phase 1 (<5 scored) no banner; Phase 2 (5–9 provisional) healthy needs ≥2 strong or ≥1 at 8.0+, BST on 0 strong, neutral on 1 job in [7.0, 7.9]; Phase 3 (10+ forced). BST/healthy mutually exclusive via `suppressAllBanners()`. Sidecard weak-job click respects classification state. DOM hydration observer confirms LinkedIn incrementally hydrates cards. Surface-validation logger fires on page-load/scoring-batch/banner-change/dom-hydration. Issue #81. Files: `extension/content_linkedin.js`.
