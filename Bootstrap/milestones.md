...
- Verify extension build stability (current version: 0.9.25)
...
2. **Surface Quality Banner** — *(removed)* External banner removed; surface intelligence state preserved in scoring pipeline.
3. **Better Search Trigger (BST)** — surface-classification-driven recovery suggestion rendered as in-sidecard collapsible "Adjacent Searches" section; must never repeat previously suggested or searched titles in a session.
...
- [x] BST renders as in-sidecard collapsible section (not popup/banner)
- [x] Executive Summary replaces Bottom Line with 3-point scannable format (fit, risk, guidance)
...
- [x] Sales/partnerships roles correctly classified as sales_execution (v0.9.23)
- [x] Work preference modulation wired through scoring pipeline (v0.9.24)
- [x] Preference modulation gated behind signals toggle (v0.9.25)

IMPLEMENTATION LOG:
- 2026-03-20: Signal Toggle Gate for Preference Modulation (v0.9.25). Preference modulation gated behind `session.includeDetectedSignals === true`. Signals OFF or null → preferences ignored, scoring identical to baseline. `signalsActive` flag added to `debug_work_mode` API response. 195 tests passing. Files: `app/api/extension/fit/route.ts`, `lib/work_mode.ts`, `lib/work_mode.test.ts`.
- 2026-03-20: Work Preference Signal Insertion Layer (v0.9.24). `WorkPreferences` type added to `CalibrationSession` (primaryMode, preferredModes[], avoidedModes[]). `applyPreferenceModulation()` applies additive adjustments: primary +0.5, secondary +0.3, avoided -1.5. Avoided takes precedence. Wired through `evaluateWorkMode()` → fit route with `preferenceAdjustment` in debug output. 16 new tests (8 unit + 8 integration). Files: `lib/work_mode.ts`, `lib/work_mode.test.ts`, `lib/calibration_types.ts`, `app/api/extension/fit/route.ts`.
- 2026-03-20: Sales/Partnerships Classification Expansion (v0.9.23). 11 new relationship-driven B2B sales signals added to SALES_TRIGGERS (partnerships, sponsorship, account growth, pipeline ownership, channel development). PRODUCT_STRATEGY_ANCHORS leakage guard prevents strategy/product roles from being pulled into sales. 5 new job fixtures (BDM, Partnerships Manager, Account Executive, Product Manager, Growth Strategy Director). 12 new tests, 107 work_mode tests. Files: `lib/work_mode.ts`, `lib/work_mode.test.ts`, `lib/__fixtures__/work_mode_fixtures.ts`.
- 2026-03-20: Sidecard UX — BST + Executive Summary. (1) BST moved from external floating popup (`cb-recovery-banner`) to in-sidecard collapsible "Adjacent Searches" section (`cb-bst-section`), hidden by default, shown on BST TRIGGER. All external banner HTML/CSS/handlers removed. (2) Bottom Line replaced with 3-point Executive Summary (`cb-execsum-section`, starts open): fit signal (green ✓, support count), risk/stretch (yellow ⚠, stretch count), decision guidance (blue →, score-band derived). (3) Dead code cleanup: unreachable `showSurfaceQualityBanner` body, dropdown dismiss handler, all `cb-recovery-*`/`cb-sq-*` references. 14 insertions, 95 deletions. Build clean, 167 tests passing (no regressions). Files: `extension/content_linkedin.js`.
- 2026-03-20: Structural vs Execution Discriminator for Work-Mode Classifier. Added post-classification discriminator to `classifyJobWorkMode` that prevents execution-heavy roles from being misclassified as builder_systems via ambiguous vocabulary overlap (infrastructure, workflow, engineering, implementation, integration, "building systems", process improvement). Two new trigger arrays: STRUCTURAL_SIGNALS (15 unambiguous tech-creation triggers like CI/CD, architecture, coding, API, deploy) and EXECUTION_MANAGEMENT_SIGNALS (20 coordination/management triggers like construction, project management, budget tracking, stakeholder management, permits, site management). Logic: when a job classifies as builder_systems but execution signals ≥4 AND execution > structural → reclassify to operational_execution. 2 new fixture jobs (construction PM, program coordinator), 19 new tests. 95 total tests passing. No existing builder jobs (Systems/Product, DevOps, Full-Stack) affected. Files: `lib/work_mode.ts`, `lib/__fixtures__/work_mode_fixtures.ts`, `lib/work_mode.test.ts`.
- 2026-03-20: Work-Mode Fixture Library Expansion. Centralized fixture module (`lib/__fixtures__/work_mode_fixtures.ts`) with 5 core user profiles (Chris=builder, Marcus=sales, Priya=ops, Fabio=analytical, Luna=creative), 5 blended crossover profiles (Alex=builder+analytical, Dana=sales+ops, Rio=creative+builder, Nadia=analytical+creative, Tomas=ops+sales), Dingus/Jen preserved as canonical weak/legacy anchors. 19 job fixtures covering all 5 modes, intensity tiers, and 3 false-positive trap jobs (VP Sales strategy vocabulary, Startup COO ownership vocabulary, Field Ops Director leadership vocabulary). 35 new tests added (9 user classification, 10 job classification, 12 e2e regression, 4 false-positive prevention). 76 total tests passing. No scoring thresholds or compatibility rules changed. Files: `lib/__fixtures__/work_mode_fixtures.ts`, `lib/work_mode.test.ts`.
...
EXTENSION RELEASE POLICY (added 2026-03-20):
- Chrome Store uploads are milestone-gated, not fix-gated.
- Internal builds on `main` iterate freely (sideloaded ZIPs for dev/local testing).
- Store submissions require: (1) major stabilization block complete, (2) `stable` branch build is beta-ready or later.
- Individual fixes/adjustments do not trigger store uploads. `main` builds are never submitted to the store.
- See `CALIBER_EXECUTION_CONTRACT.md` for full enforcement rules.
