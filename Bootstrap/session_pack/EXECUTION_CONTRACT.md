...
- Builder policy (Claude in Codespaces):
  - Make surgical edits locally; do not include uncommitted changes in the build session.
- Evidence rule:
...
- Current release model: `main` = development/preview; `stable` = production (Vercel auto-deploys from `stable`). Promote by merging `main` → `stable`.
