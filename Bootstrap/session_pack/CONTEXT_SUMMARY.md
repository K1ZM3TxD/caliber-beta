...
**Post-cache roadmap reset (2026-03-29):** Architectural direction encoded. Product has stable extension surface, working Canonical Job Cache, first web ingestion paths. Transition from DOM-centric architecture thinking to canonical job inventory + source-adapter direction. Key principles settled: (1) DOM prescan is not the acquisition path — card DOM has no JD text, prescan scores are structurally inflated, suppressed since v0.9.42. (2) Scraper-first is explicitly not the default — fragile, legally risky, deferred. (3) Canonical job inventory is the strategic backend substrate — job acquisition and job intelligence are separate concerns. (4) Inventory sources are pluggable — sidecard click, user paste, structured feed, partner API all land in the same CanonicalJob schema at different `textSource` trust levels. Next sequence: observe real usage → source-adapter interface → first structured job source → scored recommendations on /jobs. See `Bootstrap/milestones.md` Post-Beta milestone for full ordered sequence. Issue #121 opened.

**Job Source Adapter layer (2026-03-30):** Backend source-adapter layer shipped. `lib/job_source_adapter.ts` defines: `JobSourceAdapter<TRaw>` interface, six source types (`extension_sidecard`, `extension_pipeline`, `user_import`, `ats_api`, `employer_jsonld`, `licensed_feed`), trust levels (`user_verified` | `api_structured` | `feed_unverified`), `ProcessingRights` (canScore/canStore/canDisplay/canTailor), `JobProvenance` metadata, and `canonicalizeAndWrite()` entry that bridges adapter output → `writeTrustedScore`. `lib/job_source_adapters.ts` provides six concrete adapters with validation and normalization. 44 tests pass (adapter validation, normalization, cross-source convergence, rights gates, registry, textSource mapping). Existing cache/write/read flows unchanged — adapter layer is additive. Issue #121 resolved, Issue #122 opened for first structured source.
