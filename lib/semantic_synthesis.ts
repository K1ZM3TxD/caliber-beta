...
export async function generateSemanticSynthesis(args: {
  ...
  // Compute anchors from resume + prompt answers text
  const promptAnswersText = (args.promptAnswers || []).map((a) => a.answer || "").join("\n")
  const anchors = extractLexicalAnchors({ resumeText: args.resumeText || "", promptAnswersText })
  const topVerbs = anchors.verbs.slice(0, 12).map((x) => x.term)
  const topNouns = anchors.nouns.slice(0, 12).map((x) => x.term)
  const anchorTerms = [...topVerbs, ...topNouns]
  const hasAnchors = anchorTerms.length > 0
  const anchorsBlock = hasAnchors ? anchorTerms.join(", ") : "(none)"
  ...
  const userPrompt = [
    ...
    "- For the Identity + Intervention lines, reuse at least 2 anchor terms when anchors are provided.",
