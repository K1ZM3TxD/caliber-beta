...
export function validateIngestInput(
  ...
  if (typeof input.url !== "string" || input.url.trim().length === 0) {
    return { ok: false, error: "Job URL is required.", field: "url" };
  }
  ...
  try {
    ...
    return {
      ok: false,
      error: "Job URL is not valid. Paste the full URL including https://.",
      field: "url",
    };
  }
  ...
  if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
    return {
      ok: false,
      error: "Job URL must start with https:// or http://.",
      field: "url",
    };
  }
  ...
  if (PRIVATE_IP_RE.test(parsed.hostname)) {
    return {
      ok: false,
      error: "Job URL must be a public web address, not a local or private host.",
      field: "url",
    };
  }
  ...
  return { ok: true, normalizedUrl: rawUrl, normalizedText: trimmedText };
}
