...
function sortCombinedDeterministically(a: CombinedAnchorTerm, b: CombinedAnchorTerm): number {
  if (b.count !== a.count) return b.count - a.count
  if (a.term < b.term) return -1
  if (a.term > b.term) return 1
  // kind asc to keep ties deterministic
 ...
 * - combined ordering: merged ranking by (count desc, term asc, kind asc)
 *
...
export function extractLexicalAnchors(input: ExtractLexicalAnchorsInput): ExtractLexicalAnchorsOutput {
  ...
  ].sort(sortCombinedDeterministically)
