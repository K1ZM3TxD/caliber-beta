...
async function runCalibrationLoopSmoke(): Promise<void> {
  const created = await event({ type: "CREATE_SESSION" }, "CREATE_SESSION")
  ...
  // Resume -> Prompt 1
  await advanceUntil(sessionId, "PROMPT_1", 3, "TO_PROMPT_1")

  // Prompt 1..5 submissions advance server-side per event (no extra ADVANCE between prompts)
  for (let i = 1; i <= 5; i += 1) {
    const res = await event({ type: "SUBMIT_PROMPT_ANSWER", sessionId, answer: answerForPrompt(i) }, `SUBMIT_PROMPT_ANSWER_${i}`)
    if (i < 5) {
  ...
  const ritual = await advanceUntil(sessionId, "CONSOLIDATION_RITUAL", 3, "TO_RITUAL")
  assert(typeof ritual.session.consolidationRitual?.progressPct === "number", "Ritual should include progressPct")

  const synth = await advanceUntil(sessionId, "PATTERN_SYNTHESIS", 20, "TO_SYNTHESIS")
  ...
  await advanceUntil(sessionId, "TITLE_HYPOTHESIS", 3, "TO_TITLE_HYPOTHESIS")
  const titleDialogue = await advanceUntil(sessionId, "TITLE_DIALOGUE", 3, "TO_TITLE_DIALOGUE")
  ...
  const step2 = await event(
    {
      type: "SUBMIT_JOB_TEXT",
      sessionId,
      jobText: `Director of Revenue Operations (RevOps) — Enterprise, Global Organization
...
Governance, compliance, audit readiness (SOC 2 / ISO 27001), risk management, and controls experience is required.`,
  ...
  const step3 = await event({ type: "ADVANCE", sessionId }, "ADVANCE after first job")
  expectState(step3, "ALIGNMENT_OUTPUT", "Step 3 ADVANCE -> ALIGNMENT_OUTPUT")

  const step4 = await event({ type: "COMPUTE_ALIGNMENT_OUTPUT", sessionId }, "COMPUTE_ALIGNMENT_OUTPUT #1")
...
runCalibrationLoopSmoke().catch((e: unknown) => {
